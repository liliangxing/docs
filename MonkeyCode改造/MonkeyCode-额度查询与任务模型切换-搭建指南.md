...
> ⚠️ **特别提醒（切换 glm-5 等模型前必看）**：平台上存在**多个同名 / 近似名的模型**，切换前务必用 `GET /api/v1/users/models` 确认目标模型的精确 `id`，**不要只凭名字匹配**，否则可能切到错误的模型。典型例子是 glm-5 系列（随时可能变化，以下以 2026-08-11 实测为准）：

| 模型名（model 字段） | 是否隐藏（is_hidden） | 模型 id |
|----------------------|----------------------|----------|
| `glm-5` | 是（true） | `75310944-8ad1-41b0-a85a-c7b6bc8fba47` |
| `glm-5` | 否（false） | `e07243b6-4dc4-40c4-a486-a9ffa8d46c4a` |
| `glm-5.1` | 否（false） | `7e4b3ade-20b1-4b26-9a36-dc2043623a78` |
| `monkeycode-pro/glm-5` | 否（false） | `159572ce-043a-433f-b447-243843541738` |
| `monkeycode-ultra/glm-5.1` | 否（false） | `26236c05-4102-4ee3-bbe5-0f2a217cf7a8` |

> **重点**：有两个都叫 `glm-5` 的模型——一个隐藏（`75310944-…`）、一个非隐藏（`e07243b6-…`），它们的 `id` 完全不同。如果要切的是"**隐藏版 glm-5**"，请在脚本里**显式写死其 id**，而不是只传 `glm-5` 让脚本自动挑（自动挑选中"非隐藏版"的概率很高）。本文档配套脚本 `MonkeyCode改造/脚本/monkeycode_switch_model.py` 已默认锁定隐藏版 glm-5 的 id；若你其实想要另一个（如非隐藏版 `glm-5` 或 `glm-5.1`），改一下脚本里的目标模型名 / 直接指定 `MODEL_ID` 即可。
